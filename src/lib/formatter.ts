import { AfkState, HardwareStats, HeartRateState, MediaState } from '../types';
...
export function getBpmZoneName(bpm: number, lang: 'en' | 'de' = 'en'): string {
  if (lang === 'de') {
...
export function formatChatboxMessage(
  ...
  // Support both object options and legacy positional parameters
  ...
  if (afkState?.isAfk && afkOverrideChatbox) {
    message = afkTemplate;
  } else if (template === '❤️ {hr} BPM ❤️\\n{song}' || template === '❤️ {hr} BPM ❤️\n{song}') {
    // If user uses default template with song, adapt gracefully if song is empty/stopped
    message = isPlaying && fullSong ? `❤️ ${hrBpmStr} BPM ❤️\n🎵 ${fullSong} 🎵` : `❤️ ${hrBpmStr} BPM ❤️`;
  }
