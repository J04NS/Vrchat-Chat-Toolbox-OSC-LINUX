...
// File for persisting settings on Linux / local machine
const CONFIG_FILE_PATH = path.join(process.cwd(), 'chatbox-config.json');

// Masked representation
const MASKED_HYPERATE_KEY = 'Server: 164.30.71.45:7871';
...
function getEffectiveHeartRateState(): HeartRateState {
  const provider = currentConfig.heartRateProvider || 'hyperate';
...
function evaluateProfileRules(): {
  ...
  if (
    !currentConfig.profileAutomationEnabled ||
    !currentConfig.profileRules ||
    currentConfig.profileRules.length === 0
  ) {
    const activeP =
  ...
  for (const rule of currentConfig.profileRules) {
    if (!rule.enabled) continue;

    const hrMatch =
      rule.conditions.heartRate === 'any'
        ? true
...
// Check GPU stats periodically via nvidia-smi if available
...
function handleMovementDetected() {
  ...
  // If user was AFK (from timer, manual, or previous avatar signal), moving in-game turns AFK off!
  if (afkState.isAfk) {
...
function connectHyperate(sessionId: string, customRelayUrl?: string) {
  ...
  // Safely close existing websocket without triggering uncaught exceptions
  if (hyperateWs) {
  ...
  lastActiveSessionId = cleanSessionId;
  lastActiveRelayUrl = userCustomRelay;

  // Determine WebSocket target URL: Default to user's server 164.30.71.45:7871
  ...
  try {
    ...
    ws.on('open', () => {
      ...
      // 1. Send subscription payload for Pelikan relay
      try {
      ...
      // 2. Also send Phoenix channel join in case upstream phoenix protocol is mirrored
      try {
    ...
    ws.on('message', (data: WebSocket.Data) => {
      try {
        ...
        // 1. Relay BPM format: { type: 'bpm', bpm: 75 } or { bpm: 75 } or { hr: 75 }
        ...
        // 2. Direct HypeRate Phoenix events
...
let lastActivePulsoidToken: string = '';
...
function connectPulsoid(tokenInput: string) {
  ...
  if (!cleanToken) {
    ...
    lastActivePulsoidToken = '';
    return;
  }

  lastActivePulsoidToken = cleanToken;
...
// Automatic Media Detection via playerctl (Spotify / Linux MPRIS)
...
function handleMediaUpdate(status: string, artist: string, title: string, album: string, source: string) {
  ...
  if (isPlaying && (title || artist)) {
    ...
    if (hasChanged) {
      console.log(`[Media] Song erkannt (${source}): ${artist} - ${title}`);
    }
  } else {
    if (mediaState.isPlaying) {
      mediaState.isPlaying = false;
      mediaState.sourceName = status ? `${source} (${status})` : 'playerctl (idle)';
      mediaState.lastUpdated = Date.now();
...
function checkNowPlayingViaPlayerctl() {
...
function restartLoop() {
  ...
  loopTimer = setInterval(() => {
    ...
    // Update Hardware Stats if enabled
    if (currentConfig.hardwareStatsEnabled) {
      updateHardwareStats();
    }

    // AFK Timing & Inactivity Check
...
async function startServer() {
  ...
  // Connect both providers if configured, independent states
  ...
  // Start auto media polling immediately
  startAutoMediaPolling();

  // Start Hardware Stats initial reading and GPU monitor
  updateHardwareStats();
  startGpuMonitoring();

  // Start incoming VRChat OSC Listener (UDP port 9001 for AFK & Avatar Parameters)
  startVRChatOscListener();

  // Start OSC loop if enabled
  restartLoop();
  ...
  app.get('/api/status', (req, res) => {
    ...
    const response: ServerStatusResponse = {
      ...
      hyperateKeyMasked: isCustomRelay 
        ...
        : 'Server: 164.30.71.45:7871',
      hyperateConnected: hyperateState.connected,
      pulsoidConnected: pulsoidState.connected,
      heartRateProvider: currentConfig.heartRateProvider || 'hyperate',
  ...
  app.get('/api/config', (req, res) => {
    res.json({
      ...
      hyperateKeyMasked: currentConfig.hyperateRelayUrl ? 'Custom Relay' : 'HypeRate Cloud Direkt',
      heartRateProvider: currentConfig.heartRateProvider || 'hyperate',
      profileAutomationEnabled: currentConfig.profileAutomationEnabled || false,
      profileRules: currentConfig.profileRules || DEFAULT_PROFILE_RULES,
    });
  });
  ...
  app.post('/api/config', (req, res) => {
    ...
    // Re-connect HypeRate if session or relay changed
    if (
  ...
  app.delete('/api/profiles/:id', (req, res) => {
    ...
    // Update any profileRules that referenced the deleted profile
